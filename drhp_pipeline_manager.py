...
def get_all_companies_with_status():
    """Retrieves all companies that have completed processing (have markdown)."""
    companies = []
    ...
    for doc in all_company_docs:
        ...
        # Only return companies that have completed processing
        if has_markdown:
            companies.append({
                "id": str(doc.id),
                "name": doc.name,
...
def get_final_markdown(company_id):
    ...
    try:
        print(f"DEBUG: Looking for markdown for company ID: {company_id}")
        object_id = ObjectId(company_id)
        markdown_doc = FinalMarkdown.objects.get(company_id=object_id)
        print(f"DEBUG: Found markdown for company: {markdown_doc.company_name}")
        return markdown_doc.markdown
    except DoesNotExist:
        print(f"DEBUG: Markdown not found for company ID: {company_id}")
        return None
    except Exception as e:
        print(f"DEBUG: Error getting markdown for company ID {company_id}: {e}")
        return None
...
def get_company_by_id(company_id):
    ...
    try:
        ...
        print(f"DEBUG: ID type: {type(company_id)}")
        ...
        company_doc = Company.objects.get(id=object_id)
        print(f"DEBUG: Found company: {company_doc.name}")
        return company_doc
    except DoesNotExist:
        print(f"DEBUG: Company not found in database for ID: {company_id}")
        return None
    except Exception as e:
        print(f"DEBUG: Error getting company by ID: {e}")
        return None
