...
def get_company_processing_status(company_doc: Company) -> ProcessingStatus:
    ...
    return ProcessingStatus(
        ...
        checklist_completed=ChecklistOutput.objects(company_id=company_doc).first()
        is not None,
        markdown_generated=FinalMarkdown.objects(company_id=company_doc).first()
        is not None,
    )
...
def save_final_markdown(company_doc: Company, markdown_content: str):
    ...
    FinalMarkdown.objects(company_id=company_doc).update_one(
        set__company_name=company_doc.name, set__markdown=markdown_content, upsert=True
    )
    ...
    try:
        ...
        context = {
            ...
            "company_logo_data": load_image_base64(
                os.path.join(assets_dir, "Pine Labs_logo.png")
            ),
            "content": html_body,
        }
...
async def process_drhp_pipeline(pdf_path: str, queue: asyncio.Queue):
    ...
    try:
        ...
        if not ChecklistOutput.objects(
            company_id=company_doc, checklist_name=checklist_name
        ).first():
...
async def upload_and_process_drhp(file: UploadFile = File(...)):
    ...
    if not await validate_pdf_file(file):
        raise HTTPException(
            status_code=400, detail="Invalid file. Please upload a PDF under 50MB."
        )
...
def get_company_details(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get company details: {e}"
        )
...
def get_company_report(
    ...
    try:
        ...
        if not markdown_doc:
            raise HTTPException(
                status_code=404, detail="Report not found for this company"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to get company report: {e}"
        )


@app.get("/companies/{company_id}/report-html", response_model=ReportResponse)
def get_company_report_html(
    company_id: str = Path(..., description="The MongoDB ID of the company")
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to generate HTML report: {e}"
        )
